    ...
    omdbSearch: function(input){
        ...
        request(queryUrl, function(error, response, body) {
            ...
            if (!error && response.statusCode === 200) {
                ...
                fs.appendFileSync('log.txt', JSON.parse(body).Title);
                // Year of release
                console.log("Release Year: " + JSON.parse(body).Year);
                fs.appendFileSync('log.txt', JSON.parse(body).Year);
                // IMBD rating
                console.log("IMDB Rating: " + JSON.parse(body).Ratings[0].Value)
                fs.appendFileSync('log.txt', JSON.parse(body).Ratings[0].Value);
                // Rotten Tomatoes Rating
                console.log("Rotten Tomatoes Rating: " + JSON.parse(body).Ratings[1].Value)
                fs.appendFileSync('log.txt', JSON.parse(body).Ratings[1].Value);
                // Production Country
                console.log("Prodcuced in " + JSON.parse(body).Country);
                fs.appendFileSync('log.txt', JSON.parse(body).Country);
